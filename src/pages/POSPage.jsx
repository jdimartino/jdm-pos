...
export default function POSPage() {
    ...
    if (posMode === 'client-summary') {
        ...
        const handleWhatsAppSummary = () => {
            ...
            const socialLine = business?.instagram ? `\nSiguenos en ${business.instagram}` : ''
            const msg = `🍔 *${businessName}* — Detalle de tu cuenta\n\nHola *${selectedClient?.name}*, aquí el resumen:\n\n${lines}${abonoLine}\n💵 *Total a pagar: ${formatUSD(nuevoTotal)}*\n${receiptSection}\n_JDM-POS by #JDMRules_${socialLine}`
    ...
    const handleWhatsApp = async () => {
        ...
        try {
            ...
            const businessName2 = business?.name || 'JDM-POS'
            const receiptLines2 = []
            if (business?.whatsappReceipt?.holder) receiptLines2.push(`👤 ${business.whatsappReceipt.holder}`)
            if (business?.whatsappReceipt?.phone) receiptLines2.push(`📱 ${business.whatsappReceipt.phone}`)
            if (business?.whatsappReceipt?.id) receiptLines2.push(`Cedula  ${business.whatsappReceipt.id}`)
            if (business?.whatsappReceipt?.bank) receiptLines2.push(`🏦 ${business.whatsappReceipt.bank}`)
            const receiptSection2 = receiptLines2.length > 0 ? `\n*Datos del Pago Movil*\n${receiptLines2.join('\n')}\n` : ''
            const socialLine2 = business?.instagram ? `\nSiguenos en ${business.instagram}` : ''
            const msg = `🍔 *${businessName2}* — Detalle de tu cuenta\n\nHola *${client?.name}*, aquí el resumen:\n\n${lines}\n\n💵 *Total: ${formatUSD(order?.totalUSD || totalUSD)}*\n${receiptSection2}\n_JDM-POS by #JDMRules_${socialLine2}`
